...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='EATA')
    ...
    optimizer = torch.optim.Adam(params, lr=2.5e-4)
    ...
    for i in range(int(len(data_test.data)/batch)):
        if i == 0:
            y_pred = adapt_model(data_test.data[i*batch:(i+1)*batch].to(device))
        else:
    ...
    for i in range(int(len(data_test_ood.data)/batch)):
        if i == 0:
            y_pred_ood = adapt_model(data_test_ood.data[i*batch:(i+1)*batch].to(device))
        else:
            y_pred_ood = torch.cat([y_pred_ood, adapt_model(data_test_ood.data[i*batch:(i+1)*batch].to(device))],dim=0)
    y_pred_ood = torch.cat([y_pred_ood, adapt_model(data_test_ood.data[int(len(data_test_ood.data)/batch)*batch:].to(device))],dim=0)
