...
def main(args):
    ...
    num_target = 1
    if info['task_type'] == 'regression':
        syn_target = sample_con[:, :num_target]
        sample_con = sample_con[:, num_target:]
    
    else:
        print(sample_dis.shape)
        syn_target = sample_dis[:, :num_target]
        sample_dis = sample_dis[:, num_target:]
    
    save_path = args.save_path
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    np.save(save_path+'X_num_train.npy',sample_con)
    np.save(save_path+'X_cat_train.npy',sample_dis)
    np.save(save_path+'y_train.npy',syn_target)
    
    pd.DataFrame(data=sample_con).to_csv(save_path+'X_num_train.csv')
    pd.DataFrame(data=sample_dis).to_csv(save_path+'X_cat_train.csv')
