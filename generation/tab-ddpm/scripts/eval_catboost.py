...
def read_pure_data_noy(path, split='train'):
...
def train_catboost(
    ...
    # synthetic_data_path = os.path.join(parent_dir)
    # X_num_test, X_cat_test = read_pure_data_noy(synthetic_data_path)
    # y_test = np.ones(X_num_test.shape[0])
    ...
    if parent_dir is not None:
        lib.dump_json(report, os.path.join(parent_dir, eval_type + "_results_catboost.json"))

    return metrics_report

if __name__ == '__main__':
    
    raw_config = lib.load_config('/home/huangyuqing/tab-ddpm-main/exp/adult/ddpm_cb_best/config.toml')
    T_dict = deepcopy(raw_config['eval']['T'])
    
    train_catboost(
        parent_dir='/home/huangyuqing/TABSYN/synthetic/fb-comments/codi/',
        real_data_path='/home/huangyuqing/tab-ddpm-main/data/fb-comments/',
        eval_type='synthetic',
        T_dict=T_dict,
        seed=10,
        change_val=False
