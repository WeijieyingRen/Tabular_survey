...
def privacy_metrics(real_path,fake_path, data_percent=15):
    ...
    if len(y_fake.shape) == 2:
        y_fake = y_fake.squeeze(1)
...
def calc_privacy_rec(config, seed=0):
    # config = lib.load_config(config_path)
    # sample_wrapper(method, config, num_samples=config["sample"]["num_samples"], seed=seed)
    timer = zero.Timer()
    timer.run()
...
def main():
    ...
    config = {}
    config['parent_dir'] = '/home/huangyuqing/TABSYN/synthetic/fb-comments/codi/'
    config['real_data_path'] = '/home/huangyuqing/tab-ddpm-main/data/fb-comments/'
